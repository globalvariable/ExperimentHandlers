...
bool handle_robot_arm_position_threshold(ThreeDofRobot *robot, MovObjHandParadigmRobotReach *paradigm, MovObjStatus *mov_obj_status, TimeStamp current_time, MovObjHand2MovObjDurHandMsg *msgs_mov_obj_hand_2_mov_obj_dur_hand, MovObjHand2TrialHandMsg *msgs_mov_obj_hand_2_trial_hand, MovObjStatusHistory* mov_obj_status_history)
{
	...
	static bool reached_threshold_1 = 0;
	static bool reached_threshold_2 = 0;
	...
	switch (*mov_obj_status)
	{
		...
		case MOV_OBJ_STATUS_RESETTING_TO_TARGET_POINT:
			...
			if (apply_ellipsoid_threshold(&(threshold->lever_press_reach_threshold_1), tip_position->height, tip_position->depth, tip_position->lateral, target_coordinates->height, target_coordinates->depth, target_coordinates->lateral))
			{
				if (! reached_threshold_1)
				{
					if (! write_to_mov_obj_hand_2_trial_hand_msg_buffer(msgs_mov_obj_hand_2_trial_hand, current_time,  MOV_OBJ_HAND_2_TRIAL_HAND_MSG_REACHED_2_THRESHOLD_1, 0))
						return print_message(BUG_MSG ,"MovObjHandler", "HandleRobotPosition", "handle_robot_arm_position_threshold", "! write_to_mov_obj_hand_2_trial_hand_msg_buffer()");
					reached_threshold_1 = 1;
				}
			}
			if (apply_ellipsoid_threshold(&(threshold->lever_press_reach_threshold_2), tip_position->height, tip_position->depth, tip_position->lateral, target_coordinates->height, target_coordinates->depth, target_coordinates->lateral))
			{
				if (! reached_threshold_2)
				{
			...
			{
				reached_threshold_1 = 0;
				reached_threshold_2 = 0;
				reached_target = 0;		
			}
