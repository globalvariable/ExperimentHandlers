			...
			case MOV_OBJ_HAND_2_TRIAL_HAND_MSG_REACHED_2_TARGET:     // mov obj handler sends this when it receives a trial start msg from trial hand			
				switch (*trial_status)
				{
					...
					case TRIAL_STATUS_IN_TRIAL:
						...
						if (paradigm->current_trial_data.target_led_component_list_idx == 1)  // left target is go, right target is no-go trial, it is right target
						{
							...
							if (!write_to_trial_hand_2_trial_dur_hand_msg_buffer(msgs_trial_hand_2_trial_dur_hand, current_time, TRIAL_HAND_2_TRIAL_DUR_HAND_MSG_ENABLE_DURATION_HANDLING, current_time + paradigm->no_go_trial_wait_at_target_duration))
								return print_message(ERROR_MSG ,"TrialHandler", "HandleTrialDurHand2TrialHandMsgs", "handle_trial_dur_handler_to_trial_handler_msg", "write_to_trial_hand_2_trial_dur_hand_msg_buffer()");							
						}
						else
						{
