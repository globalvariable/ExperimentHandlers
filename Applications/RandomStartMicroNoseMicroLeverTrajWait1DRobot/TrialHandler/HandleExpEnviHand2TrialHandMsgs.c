			...
			case EXP_ENVI_HAND_2_TRIAL_HAND_MSG_REWARD_REQUEST:	
				switch (*trial_status)
				{
					...
					case TRIAL_STATUS_REWARD_AVAILABLE:
						if (paradigm->current_trial_data.target_led_component_list_idx != paradigm->current_trial_data.robot_target_position_idx)	// it is a catch trial
							break;
						*trial_status = TRIAL_STATUS_IN_REFRACTORY;
						...
						{
							if (get_rand_number() < 0.5)
							{ 
								if (paradigm->current_trial_data.robot_target_position_idx == 0)
						...
						paradigm->current_trial_data.robot_start_position_idx = (unsigned int)   (((paradigm->num_of_robot_start_positions + 1) / 2) * get_rand_number());   ///  Bunu trial bittiginde yap.
				
						if (paradigm->current_trial_data.robot_target_position_idx == 1)
							paradigm->current_trial_data.robot_start_position_idx  = paradigm->current_trial_data.robot_start_position_idx;
						else
							paradigm->current_trial_data.robot_start_position_idx  = paradigm->num_of_robot_start_positions - 1 - paradigm->current_trial_data.robot_start_position_idx;
