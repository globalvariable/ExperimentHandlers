...
bool handle_exp_envi_tx_shm_and_send_rs232_pulse_width_command(TimeStamp current_time, ThreeDofRobotPulseHistory *robot_pulse_history, double max_servo_angle_change, unsigned int servo_degree_handling_period_multiplier, double servo_angle_change_threshold)
{
	unsigned int i;
	ExpEnviTxShm exp_envi_tx_buffer;
	unsigned char cmd_low_byte, cmd_high_byte;
	static unsigned int period_multiplier = 0;
	...
	{
		period_multiplier++;
		if (period_multiplier == servo_degree_handling_period_multiplier)
		{
			period_multiplier = 0;
			evaluate_three_dof_robot_arm_pw_command_with_degree_limitation_and_threshold(static_robot_arm, max_servo_angle_change, servo_angle_change_threshold);
		}
