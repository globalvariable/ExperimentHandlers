			...
			case MOV_OBJ_DUR_HAND_2_MOV_OBJ_HAND_MSG_TIMEOUT:
				switch (msg_item.additional_data)
				{
					...
					case MOV_OBJ_DUR_STATUS_ITEM_SEND_PULSE_WIDTH:
						if (*mov_obj_status == MOV_OBJ_STATUS_RESETTING_TO_TARGET_POINT)
						{
							
							if (mov_obj_paradigm->random_traj_mode)
							{
								cntr++;
								if  (cntr == 8)
								{
									...
									idx  = mov_obj_paradigm->target_info.selected_position_idx;
									if (get_rand_number() < mov_obj_paradigm->randomization_level)
										idx = ! idx;
									submit_servo_target(&(robot_arm->servos[BASE_SERVO]), mov_obj_paradigm->target_info.robot_pulse_widths[idx].pulse[BASE_SERVO], speed);
								}
							}
						}
						if (! handle_exp_envi_tx_shm_and_send_rs232_pulse_width_command(current_time, robot_pulse_history, mov_obj_status, mov_obj_status_history, msgs_mov_obj_hand_2_trial_hand))
