...
static void submit_trial_duration_button_func (void);
static void submit_trial_refractory_button_func (void);


static void increase_threshold_button_func (void);
static void decrease_threshold_button_func (void);

static GtkWidget *entry_trial_duration;
static GtkWidget *btn_submit_trial_duration;

static GtkWidget *entry_trial_refractory;
static GtkWidget *btn_submit_trial_refractory;
...
bool create_trial_handler_tab(GtkWidget *tabs, RtTasksData *rt_tasks_data, Gui2TrialHandMsg *msgs_gui_2_trial_hand, TrialHandParadigmRobotReach *trial_hand_paradigm, ClassifiedTrialHistory* classified_trial_history, TrialHand2GuiMsg *msgs_trial_hand_2_gui, TrialStatusHistory *trial_status_history)
{
	...
	btn_submit_trial_duration = gtk_button_new_with_label("Trial Duration");
	gtk_box_pack_start (GTK_BOX (hbox), btn_submit_trial_duration, FALSE, FALSE, 0);

	entry_trial_duration = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox), entry_trial_duration, FALSE, FALSE, 0);
	gtk_widget_set_size_request(entry_trial_duration, 50, 25);
	gtk_entry_set_text(GTK_ENTRY(entry_trial_duration), "100");
	...
	btn_submit_trial_refractory = gtk_button_new_with_label("Trial Refractory");
	gtk_box_pack_start (GTK_BOX (hbox), btn_submit_trial_refractory, FALSE, FALSE, 0);

	entry_trial_refractory = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox), entry_trial_refractory, FALSE, FALSE, 0);
	gtk_widget_set_size_request(entry_trial_refractory, 50, 25);
	gtk_entry_set_text(GTK_ENTRY(entry_trial_refractory), "100");
	lbl = gtk_label_new("ms");	
     	gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE, FALSE, 0);
	...
	g_signal_connect(G_OBJECT(btn_submit_trial_duration), "clicked", G_CALLBACK( submit_trial_duration_button_func), NULL);
	g_signal_connect(G_OBJECT(btn_submit_trial_refractory), "clicked", G_CALLBACK( submit_trial_refractory_button_func), NULL);
...
static void submit_trial_duration_button_func (void)
{
	TimeStamp trial_duration;

	trial_duration = (TimeStamp)(1000000.0 * atof(gtk_entry_get_text(GTK_ENTRY(entry_trial_duration))));

	paradigm->max_trial_length = trial_duration;
...
static void submit_trial_refractory_button_func (void)
{
	...
	trial_refractory = (TimeStamp)(1000000.0 * atof(gtk_entry_get_text(GTK_ENTRY(entry_trial_refractory))));

	paradigm->min_trial_refractory = trial_refractory;
