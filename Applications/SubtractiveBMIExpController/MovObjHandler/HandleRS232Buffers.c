...
bool handle_exp_envi_tx_shm_and_send_rs232_pulse_width_command(TimeStamp current_time, ThreeDofRobotPulseHistory *robot_pulse_history, double max_servo_angle_change, double bias_2_left, double servo_angle_change_threshold)
{
	unsigned int i;
	ExpEnviTxShm exp_envi_tx_buffer;
	...
	if ( check_three_dof_robot_security_limits(static_robot_arm))
	{
		for (i = 0; i < THREE_DOF_ROBOT_NUM_OF_SERVOS; i++)
			evaluate_servo_pw_command_via_spike_count_with_limitation_thresholding_biasing(&(static_robot_arm->servos[i]), max_servo_angle_change, servo_angle_change_threshold, bias_2_left);
