	...
	while (get_next_trial_hand_2_exp_envi_hand_msg_buffer_item(msgs_trial_hand_2_exp_envi_hand, &msg_item))
	{
		...
		switch (msg_item.msg_type)
		{
			case TRIAL_HAND_2_EXP_ENVI_HAND_MSG_START_TRIAL:	
				disable_exp_envi_input_with_no_status_reset(&(exp_envi_data->inp_comp_types[IR_BEAM_IDX_IN_EXP_ENVI_DATA]),  &cancellation_required_for_status_timer) ;
			...
			case TRIAL_HAND_2_EXP_ENVI_HAND_MSG_END_TRIAL:	
				...
				disable_exp_envi_input_with_no_status_reset(&(exp_envi_data->inp_comp_types[NOSE_RETRACT_IR_IDX_IN_EXP_ENVI_DATA]),  &cancellation_required_for_status_timer) ;
				if (cancellation_required_for_status_timer)
				{
