...
bool handle_exp_envi_tx_shm_and_send_rs232_pulse_width_command(TimeStamp current_time, ThreeDofRobotPulseHistory *robot_pulse_history, MovObjStatus *mov_obj_status, MovObjStatusHistory* mov_obj_status_history, MovObjHand2TrialHandMsg *msgs_mov_obj_hand_2_trial_hand)
{
	...
	{
		if (evaluate_three_dof_robot_arm_pw_command_with_any_servo_target_reach_feedback(static_robot_arm))
		{
			switch (*mov_obj_status)
			{
				...
				case MOV_OBJ_STATUS_RESETTING_TO_START_POINT:
					break;
				default: 
					return print_message(BUG_MSG ,"MovObjHandler", "HandleMovObjDurHand2MovObjHandMsgs", "handle_exp_envi_tx_shm_and_send_rs232_pulse_width_command", "switch- default");							
			}
		}	
