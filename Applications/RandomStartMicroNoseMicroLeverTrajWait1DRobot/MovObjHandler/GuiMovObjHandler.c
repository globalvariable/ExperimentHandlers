...
static GtkWidget *btn_random_traj_mode_on_off;
static GtkWidget *entry_trajectory_randomization_level;
...
static void random_traj_mode_on_off_button_func (void);
...
bool create_mov_obj_handler_tab(GtkWidget *tabs, RtTasksData *rt_tasks_data, Gui2MovObjHandMsg *msgs_gui_2_mov_obj_hand, MovObjHand2GuiMsg *msgs_mov_obj_hand_2_gui, ThreeDofRobot *robot_arm, MovObjHandParadigmRobotReach *mov_obj_paradigm, MovObjStatusHistory* mov_obj_status_history, ThreeDofRobotAngleHistory *robot_angle_history, ThreeDofRobotPulseHistory *robot_pulse_history)
{
	...
	btn_random_traj_mode_on_off = gtk_button_new_with_label("Random Trajectory: OFF");
	gtk_box_pack_start (GTK_BOX (hbox), btn_random_traj_mode_on_off, FALSE, FALSE, 0);

	lbl = gtk_label_new("level:");	
     	gtk_box_pack_start(GTK_BOX(hbox),lbl, FALSE, FALSE, 0);

	entry_trajectory_randomization_level = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox), entry_trajectory_randomization_level, FALSE, FALSE, 0);
	gtk_widget_set_size_request(entry_trajectory_randomization_level, 50, 25);
	gtk_entry_set_text(GTK_ENTRY(entry_trajectory_randomization_level), "0.1");
	...
	g_signal_connect(G_OBJECT(btn_random_traj_mode_on_off), "clicked", G_CALLBACK(random_traj_mode_on_off_button_func), NULL);
...
static void random_traj_mode_on_off_button_func (void)
{
	static_mov_obj_paradigm->randomization_level = atof(gtk_entry_get_text(GTK_ENTRY(entry_trajectory_randomization_level)));
	if (static_mov_obj_paradigm->random_traj_mode)
	{
		static_mov_obj_paradigm->random_traj_mode = FALSE;
		gtk_button_set_label (GTK_BUTTON (btn_random_traj_mode_on_off),"Random Trajectory: OFF");		
	}
	else 
	{
		static_mov_obj_paradigm->random_traj_mode = TRUE;
		gtk_button_set_label (GTK_BUTTON (btn_random_traj_mode_on_off),"Random Trajectory: ON");		
	}
}
