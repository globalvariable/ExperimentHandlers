...
int main( int argc, char *argv[])
{
	...
	if (! add_input_component_type_to_exp_envi_data(exp_envi_data, EXP_ENVI_INPUT_COMPONENT_IR_BEAM_NOSE_POKE, EXP_ENVI_INPUT_STATUS_LOW, 1000000000, 0, 1000000000, 0, 2, 1)) /// 2 nose poke
		return print_message(ERROR_MSG ,"BMIExpController", "ExpEnviHandler", "main", "! add_input_component_type_to_exp_envi_data().");
	if (! add_input_component_type_to_exp_envi_data(exp_envi_data, EXP_ENVI_INPUT_COMPONENT_LEFT_LEVER, EXP_ENVI_INPUT_STATUS_LOW, 1000000000, 100000000, 0, 0, 1, 0))  /// 100 ms lever press
		return print_message(ERROR_MSG ,"BMIExpController", "ExpEnviHandler", "main", "! add_input_component_type_to_exp_envi_data().");
	if (! add_input_component_type_to_exp_envi_data(exp_envi_data, EXP_ENVI_INPUT_COMPONENT_RIGHT_LEVER, EXP_ENVI_INPUT_STATUS_LOW, 1000000000, 100000000, 0, 0, 1, 0))  /// 100 ms lever press
		return print_message(ERROR_MSG ,"BMIExpController", "ExpEnviHandler", "main", "! add_input_component_type_to_exp_envi_data().");
