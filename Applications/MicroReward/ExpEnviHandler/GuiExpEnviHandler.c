...
static GtkWidget *entry_nose_press_duration;
static GtkWidget *btn_submit_nose_press_duration;
...
static void submit_nose_press_duration_button_func (void);
...
bool create_exp_envi_handler_tab(GtkWidget *tabs, RtTasksData *rt_tasks_data, Gui2ExpEnviHandMsg *msgs_gui_2_exp_envi_hand, ExpEnviHand2GuiMsg *msgs_exp_envi_hand_2_gui, ExpEnviInputStatusHistory *exp_envi_input_status_history, ExpEnviOutputStatusHistory *exp_envi_output_status_history, ExpEnviData *exp_envi_data, ExpEnviHandParadigmRobotReach *exp_envi_paradigm)
{
	...
	btn_submit_nose_press_duration = gtk_button_new_with_label("Nose Press Duration");
	gtk_box_pack_start (GTK_BOX (hbox), btn_submit_nose_press_duration, FALSE, FALSE, 0);

	entry_nose_press_duration = gtk_entry_new();
        gtk_box_pack_start(GTK_BOX(hbox), entry_nose_press_duration, FALSE, FALSE, 0);
	gtk_widget_set_size_request(entry_nose_press_duration, 50, 25);
	gtk_entry_set_text(GTK_ENTRY(entry_nose_press_duration), "100");
	...
	g_signal_connect(G_OBJECT(btn_submit_nose_press_duration), "clicked", G_CALLBACK(submit_nose_press_duration_button_func), NULL);
...
static void submit_nose_press_duration_button_func (void)
{
	TimeStamp nose_press_duration;
	unsigned int idx;

	nose_press_duration = (TimeStamp)(1000000.0 * atof(gtk_entry_get_text(GTK_ENTRY(entry_nose_press_duration))));

	if (! get_input_component_type_idx_in_exp_envi_data(static_exp_envi_data, EXP_ENVI_INPUT_COMPONENT_IR_BEAM_NOSE_POKE, &idx))
		return (void)print_message(ERROR_MSG ,"ExpEnviHandler", "GuiExpEnviHandler", "submit_nose_press_duration_button_func", "! get_input_component_type_idx_in_exp_envi_data()");

	if (! submit_new_min_low_status_duration_for_input_component_type(static_exp_envi_data, EXP_ENVI_INPUT_COMPONENT_IR_BEAM_NOSE_POKE, nose_press_duration))
		return (void)print_message(ERROR_MSG ,"ExpEnviHandler", "GuiExpEnviHandler", "submit_nose_press_duration_button_func", "! submit_new_min_low_status_duration_for_input_component_type");
