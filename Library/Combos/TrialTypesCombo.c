...
bool update_trial_types_combo(TrialTypesData *trial_types_data, TrialTypesCombo *combo)
{
	...
	{
		if (!get_trial_type_string(trial_types_data[i].type, temp))
			return print_message(ERROR_MSG ,"TrialControllers", "TrialTypesCombo", "update_trial_types_combo", "! get_trial_type_string.");
	 	gtk_combo_box_append_text(GTK_COMBO_BOX(combo->combo), &temp[29]);    // ignore uncessary string part
	} 
	combo->num_of_texts = trials_data->trial_types_data.num_of_types;
