...
static void *rt_trial_duration_handler(void *args)
{
	...
	if (! check_rt_task_specs_to_init(static_rt_tasks_data, TRIAL_DURATION_HANDLER_CPU_ID, TRIAL_DURATION_HANDLER_CPU_THREAD_ID, TRIAL_DURATION_HANDLER_PERIOD))  {
	...
	if (! write_rt_task_specs_to_rt_tasks_data(static_rt_tasks_data, TRIAL_DURATION_HANDLER_CPU_ID, TRIAL_DURATION_HANDLER_CPU_THREAD_ID, TRIAL_DURATION_HANDLER_PERIOD, TRIAL_DURATION_HANDLER_POSITIVE_JITTER_THRES, TRIAL_DURATION_HANDLER_NEGATIVE_JITTER_THRES))  {
        ...
        while (rt_trial_duration_handler_stay_alive) 
	{
		...
		evaluate_and_save_jitter(static_rt_tasks_data, TRIAL_DURATION_HANDLER_CPU_ID, TRIAL_DURATION_HANDLER_CPU_THREAD_ID, prev_time, curr_time);
		prev_time = curr_time;
		...
		if (! handle_trial_handler_duration(&trial_duration_status, curr_system_time, handling_end_time, static_msgs_trial_dur_hand_2_trial_hand)) {
			print_message(ERROR_MSG ,"BMIExpController", "TrialDurationHandlerRtTask", "rt_trial_duration_handler", "! handle_trial_handler_to_trial_duration_handler_msg()."); break; }
		// routines	
		evaluate_and_save_period_run_time(static_rt_tasks_data, TRIAL_DURATION_HANDLER_CPU_ID, TRIAL_DURATION_HANDLER_CPU_THREAD_ID, curr_time, rt_get_cpu_time_ns());		
        }
