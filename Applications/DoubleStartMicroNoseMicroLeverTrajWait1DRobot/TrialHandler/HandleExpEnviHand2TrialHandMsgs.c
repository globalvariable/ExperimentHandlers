				...
				switch (*trial_status)
				{
					...
					case TRIAL_STATUS_START_TRIAL_AVAILABLE:	
						*trial_status = TRIAL_STATUS_GET_READY_TO_START;
						...
						paradigm->current_trial_data.catch_trial = FALSE;

						if (paradigm->current_trial_data.restart_escaping_mode)
						{
							if (paradigm->current_trial_data.restart_escaping)
							{
								paradigm->current_trial_data.restart_escaping = FALSE;
							...
							else
							{
								...
								if (paradigm->current_trial_data.full_catch_trial_mode_on)  // exactly catch. 
									temp = 1.0;
								else
									temp = 0.5;
								...
								{
									if (get_rand_number() < temp)
										paradigm->current_trial_data.catch_trial = TRUE;
								}

								if (paradigm->current_trial_data.catch_trial )
								{
									if (paradigm->current_trial_data.target_led_component_list_idx == 0)
						...
						break;
					case TRIAL_STATUS_ESCAPING:
						break;  					
					case TRIAL_STATUS_AFTER_ESCAPING:
						break;  	
					default:
				...
				switch (*trial_status)
				{
					...
					case TRIAL_STATUS_IN_TRIAL:
						*trial_status = TRIAL_STATUS_IN_REFRACTORY;
						...
						if (!write_to_trial_hand_2_exp_envi_hand_msg_buffer(msgs_trial_hand_2_exp_envi_hand, current_time, TRIAL_HAND_2_EXP_ENVI_HAND_MSG_END_TRIAL_WHEN_GET_READY, 0))
							return print_message(ERROR_MSG ,"TrialHandler", "HandleExpEnviHand2TrialHandMsgss", "handle_exp_envi_handler_to_trial_handler_msg", "write_to_trial_hand_2_exp_envi_hand_msg_buffer()");
...
*/
					...
					case TRIAL_STATUS_ESCAPING:	
						if (paradigm->current_trial_data.restart_escaping_mode)
						{
							*trial_status = TRIAL_STATUS_IN_REFRACTORY;
							...
							paradigm->current_trial_data.restart_escaping = TRUE;
							...
							refractory = current_time + 100000000;
							if (!write_to_trial_hand_2_trial_dur_hand_msg_buffer(msgs_trial_hand_2_trial_dur_hand, current_time, TRIAL_HAND_2_TRIAL_DUR_HAND_MSG_ENABLE_DURATION_HANDLING, refractory))
									return print_message(ERROR_MSG ,"TrialHandler", "HandleExpEnviHand2TrialHandMsgs", "handle_exp_envi_handler_to_trial_handler_msg", "write_to_trial_hand_2_trial_dur_hand_msg_buffer()");
	...
	*/
						...
						{
							*trial_status = TRIAL_STATUS_AFTER_ESCAPING;							
							trial_hand_to_mov_obj_hand_msg_add.robot_target_position_idx = paradigm->current_trial_data.target_led_component_list_idx;
							if (!write_to_trial_hand_2_mov_obj_hand_msg_buffer(msgs_trial_hand_2_mov_obj_hand, current_time, TRIAL_HAND_2_MOV_OBJ_HAND_MSG_CHANGE_TARGET, trial_hand_to_mov_obj_hand_msg_add))
								return print_message(ERROR_MSG ,"TrialHandler", "HandleTrialDurHand2TrialHandMsgss", "handle_trial_dur_handler_to_trial_handler_msg", "write_to_trial_hand_2_mov_obj_hand_msg_buffer()");
							if (!write_to_trial_hand_2_exp_envi_hand_msg_buffer(msgs_trial_hand_2_exp_envi_hand, current_time, TRIAL_HAND_2_EXP_ENVI_HAND_MSG_TURN_BUZZER_OFF, 0))
								return print_message(ERROR_MSG ,"TrialHandler", "HandleExpEnviHand2TrialHandMsgss", "handle_exp_envi_handler_to_trial_handler_msg", "write_to_trial_hand_2_exp_envi_hand_msg_buffer()");
						}
						break;  
					case TRIAL_STATUS_AFTER_ESCAPING:
