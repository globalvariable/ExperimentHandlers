...
bool handle_exp_envi_handler_duration(ExpEnviInputsMinDurationStatus *exp_envi_inputs_min_dur_status, ExpEnviInputsMaxDurationStatus *exp_envi_inputs_max_dur_status, TimeStamp current_time, TimeStamp *inputs_handling_end_time_min, TimeStamp *inputs_handling_end_time_max, unsigned int num_of_input_components, ExpEnviDurHand2ExpEnviHandMsg *msgs_exp_envi_dur_hand_2_exp_envi_hand)
{
	char str_min_dur_status[EXP_ENVI_INPUTS_MIN_DUR_STATUS_MAX_STRING_LENGTH];
	char str_max_dur_status[EXP_ENVI_INPUTS_MAX_DUR_STATUS_MAX_STRING_LENGTH];
	unsigned int i;
	for (i = 0; i < num_of_input_components; i ++)
	{
		switch (exp_envi_inputs_min_dur_status[i])
		{
			...
			case EXP_ENVI_INPUTS_MIN_DUR_STATUS_TIMER_ON:
				...
				break;	// do nothing
			default: 
				get_exp_envi_input_min_dur_status_type_string(exp_envi_inputs_min_dur_status[i], str_min_dur_status);  
				return print_message(BUG_MSG ,"ExpEnviHandler", "HandleExpEnviHandDur", "handle_exp_envi_handler_duration", str_min_dur_status);		
		}
		switch (exp_envi_inputs_max_dur_status[i])
		{
			...
			case EXP_ENVI_INPUTS_MAX_DUR_STATUS_TIMER_ON:
				...
				break;	// do nothing
			default: 
				get_exp_envi_input_max_dur_status_type_string(exp_envi_inputs_max_dur_status[i], str_max_dur_status);  
				return print_message(BUG_MSG ,"ExpEnviHandler", "HandleExpEnviHandDur", "handle_exp_envi_handler_duration", str_max_dur_status);
		}
	}
