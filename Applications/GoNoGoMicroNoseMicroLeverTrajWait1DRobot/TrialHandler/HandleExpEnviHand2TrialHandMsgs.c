			...
			case EXP_ENVI_HAND_2_TRIAL_HAND_MSG_REWARD_REQUEST:	
				switch (*trial_status)
				{
					case TRIAL_STATUS_TRIALS_DISABLED:
						break;   // do nothing
					case TRIAL_STATUS_IN_TRIAL:
						if (paradigm->current_trial_data.robot_target_position_idx != paradigm->current_trial_data.target_led_component_list_idx)   // it is a catch trial					
							break;
			...
			case EXP_ENVI_HAND_2_TRIAL_HAND_MSG_NOSE_POKE_EVENT:
				switch (*trial_status)
				{
					...
					case TRIAL_STATUS_IN_TRIAL:
						if (paradigm->current_trial_data.robot_target_position_idx != paradigm->current_trial_data.target_led_component_list_idx)   // it is a catch trial					
							break;
						if (paradigm->current_trial_data.target_led_component_list_idx == 0)  // left target is go, right target is no-go trial
							break;
