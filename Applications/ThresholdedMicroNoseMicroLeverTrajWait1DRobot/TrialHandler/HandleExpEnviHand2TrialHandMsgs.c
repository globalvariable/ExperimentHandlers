...
*/
						...
						break;   // do nothing
					case TRIAL_STATUS_WAIT_FOR_LEVER_PRESS:	
						if (paradigm->current_trial_data.target_led_component_list_idx == paradigm->current_trial_data.robot_target_position_idx)	// it is not a catch trial
							break;
						*trial_status = TRIAL_STATUS_IN_REFRACTORY;
						...
						if (!write_to_trial_hand_2_trial_dur_hand_msg_buffer(msgs_trial_hand_2_trial_dur_hand, current_time, TRIAL_HAND_2_TRIAL_DUR_HAND_MSG_DISABLE_DURATION_HANDLING, 0))
							return print_message(ERROR_MSG ,"TrialHandler", "HandleExpEnviHand2TrialHandMsgs", "handle_exp_envi_handler_to_trial_handler_msg", "write_to_trial_hand_2_trial_dur_hand_msg_buffer()");
						refractory = 200000000;
						if (!write_to_trial_hand_2_trial_dur_hand_msg_buffer(msgs_trial_hand_2_trial_dur_hand, current_time, TRIAL_HAND_2_TRIAL_DUR_HAND_MSG_ENABLE_DURATION_HANDLING, refractory))
							return print_message(ERROR_MSG ,"TrialHandler", "HandleExpEnviHand2TrialHandMsgs", "handle_exp_envi_handler_to_trial_handler_msg", "write_to_trial_hand_2_trial_dur_hand_msg_buffer()");


						paradigm->current_trial_data.robot_target_position_idx = paradigm->current_trial_data.target_led_component_list_idx;   // no catch trial and target change as reward 

/*
						if (paradigm->current_trial_data.auto_target_select_mode_on)    // no catch trial and target change as reward 
							paradigm->current_trial_data.target_led_component_list_idx = (unsigned int)(paradigm->num_of_robot_target_positions * get_rand_number());   
						else
							paradigm->current_trial_data.target_led_component_list_idx = paradigm->current_trial_data.gui_selected_target_position_idx;
