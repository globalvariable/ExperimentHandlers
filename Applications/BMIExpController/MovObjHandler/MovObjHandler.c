...
int main( int argc, char *argv[])
{
	...
	if (!add_component_type_to_mov_obj_data(mov_obj_data, MOV_OBJ_COMPONENT_1D_ACTUATOR, 100000000, 500000000, 40.0, 5, 80.0, 100000000, MOV_OBJ_DIRECTION_UNSPECIFIED))  // 40 mm initial threshold
		return print_message(ERROR_MSG ,"MovObjHandler", "MovObjHandler", "main", "! add_component_type_to_mov_obj_data().");
	if (!set_global_constraints_mov_obj_data(mov_obj_data, 100000000, 500000000, 40, 5, 80.0, 100000000, MOV_OBJ_DIRECTION_UNSPECIFIED))  // 40 mm initial threshold  // this MovObjHandler acts according to global constraints, ignores component constraints
