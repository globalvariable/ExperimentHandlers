			...
			case EXP_ENVI_DUR_HAND_2_EXP_ENVI_HAND_MSG_INPUT_TIMEOUT_FOR_MIN:	
				switch (msg_item.comp_num)
				{
					...
					case LEVER_IDX_IN_EXP_ENVI_DATA:
						if (! time_out_success_for_input_comp(&(exp_envi_data->inp_comp_types[LEVER_IDX_IN_EXP_ENVI_DATA]), &has_response))
						...
						if (has_response)
						{
							print_message(INFO_MSG ,"ExpEnviHandler", "HandleExpEnviDurHand2ExpEnviHandMsgs", "handle_exp_envi_dur_handler_to_exp_envi_handler_msg", "LEVER_IDX_IN_EXP_ENVI_DATA RESPONSE");
			...
			case EXP_ENVI_DUR_HAND_2_EXP_ENVI_HAND_MSG_INPUT_TIMEOUT_FOR_MAX:	
				switch (msg_item.comp_num)
				{
					case IR_BEAM_IDX_IN_EXP_ENVI_DATA:
						...
						break;			
					case LEVER_IDX_IN_EXP_ENVI_DATA:
						reset_exp_envi_input_with_status_reset(&(exp_envi_data->inp_comp_types[LEVER_IDX_IN_EXP_ENVI_DATA]), &timer_cancellation_required);
						{
							// not required to cancel timer here. it cancels itself when there is a timeout.	
						}
						break;	
