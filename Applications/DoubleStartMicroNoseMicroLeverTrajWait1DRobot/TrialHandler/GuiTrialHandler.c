...
static void full_catch_trial_mode_on_off_button_func(void);
static void select_target_button_func (void);
static void release_reward_button_func (void);
static void end_trial_button_func (void);
...
bool create_trial_handler_tab(GtkWidget *tabs, RtTasksData *rt_tasks_data, Gui2TrialHandMsg *msgs_gui_2_trial_hand, TrialHandParadigmRobotReach *trial_hand_paradigm, ClassifiedTrialHistory* classified_trial_history, TrialHand2GuiMsg *msgs_trial_hand_2_gui, TrialStatusHistory *trial_status_history)
{
	...
	btn_full_catch_trial_mode_on_off = gtk_button_new_with_label("Full Catch Trial: OFF");
	gtk_box_pack_start (GTK_BOX (hbox), btn_full_catch_trial_mode_on_off , TRUE, TRUE, 0);
	...
	btn_end_trial = gtk_button_new_with_label("END TRIAL");
	gtk_box_pack_start (GTK_BOX (hbox), btn_end_trial , TRUE, TRUE, 0);
	...
	g_signal_connect(G_OBJECT(btn_full_catch_trial_mode_on_off), "clicked", G_CALLBACK(full_catch_trial_mode_on_off_button_func), NULL);

	g_signal_connect(G_OBJECT(btn_release_reward), "clicked", G_CALLBACK(release_reward_button_func), NULL);

	g_signal_connect(G_OBJECT(btn_end_trial), "clicked", G_CALLBACK(end_trial_button_func), NULL);
...
static void full_catch_trial_mode_on_off_button_func(void)
{
	if (paradigm->current_trial_data.full_catch_trial_mode_on)
	{
		if (!write_to_gui_2_trial_hand_msg_buffer(static_msgs_gui_2_trial_hand, static_rt_tasks_data->current_system_time, GUI_2_TRIAL_HAND_MSG_FULL_CATCH_TRIAL_MODE_OFF, 0))
			return (void)print_message(ERROR_MSG ,"TrialHandler", "GuiTrialHandler", "catch_trial_mode_on_off_button_func", "! write_to_gui_2_trial_hand_msg_buffer().");
		gtk_button_set_label (GTK_BUTTON (btn_full_catch_trial_mode_on_off),"Full Catch Trial: OFF");		
	}
	else
	{
		if (!write_to_gui_2_trial_hand_msg_buffer(static_msgs_gui_2_trial_hand, static_rt_tasks_data->current_system_time, GUI_2_TRIAL_HAND_MSG_FULL_CATCH_TRIAL_MODE_ON, 0))
			return (void)print_message(ERROR_MSG ,"TrialHandler", "GuiTrialHandler", "catch_trial_mode_on_off_button_func", "! write_to_gui_2_trial_hand_msg_buffer().");	
		gtk_button_set_label (GTK_BUTTON (btn_full_catch_trial_mode_on_off),"Full Catch Trial: ON");		
	}	
}
...
static void end_trial_button_func (void)
{
	if (!write_to_gui_2_trial_hand_msg_buffer(static_msgs_gui_2_trial_hand, static_rt_tasks_data->current_system_time, GUI_2_TRIAL_HAND_MSG_END_TRIAL, 0))
		return (void)print_message(ERROR_MSG ,"TrialHandler", "GuiTrialHandler", "start_recording_button_func ", "! write_to_gui_2_trial_hand_msg_buffer().");	
}
