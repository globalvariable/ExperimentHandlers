			...
			case EXP_ENVI_DUR_HAND_2_EXP_ENVI_HAND_MSG_INPUT_TIMEOUT_FOR_MIN:	
				switch (msg_item.comp_num)
				{
						...
						if (has_response)
						{
							...
							switch (exp_envi_paradigm->target_led_component_indexes_list[exp_envi_paradigm->selected_target_led_component_list_idx])
							{
								...
								case RIGHT_LED_IDX_IN_EXP_ENVI_DATA:
									...
									break;
								default:
									return print_message(BUG_MSG ,"ExpEnviHandler", "HandleExpEnviDurHand2ExpEnviHandMsgs", "handle_exp_envi_dur_handler_to_exp_envi_handler_msg", "EXP_ENVI_DUR_HAND_2_EXP_ENVI_HAND_MSG_INPUT_TIMEOUT_FOR_MIN - switch (exp_envi_paradigm.target_led_comp_idx) -default");	
							}
						} 
						 break;	
					...
					case NOSE_RETRACT_IR_IDX_IN_EXP_ENVI_DATA:
						if (! time_out_success_for_input_comp(&(exp_envi_data->inp_comp_types[NOSE_RETRACT_IR_IDX_IN_EXP_ENVI_DATA]), &has_response))
							return print_message(BUG_MSG ,"ExpEnviHandler", "HandleExpEnviDurHand2ExpEnviHandMsgs", "handle_exp_envi_dur_handler_to_exp_envi_handler_msg", "time_out_success_for_input_comp().");
						if (has_response)
						{
			...
			case EXP_ENVI_DUR_HAND_2_EXP_ENVI_HAND_MSG_INPUT_TIMEOUT_FOR_MAX:	
				switch (msg_item.comp_num)
				{
					...
					case RIGHT_LEVER_IDX_IN_EXP_ENVI_DATA:
						...
						break;
					case NOSE_RETRACT_IR_IDX_IN_EXP_ENVI_DATA:
						reset_exp_envi_input_with_status_reset(&(exp_envi_data->inp_comp_types[NOSE_RETRACT_IR_IDX_IN_EXP_ENVI_DATA]), &timer_cancellation_required);
						if (timer_cancellation_required)
						{
							// not required to cancel timer here. it cancels itself when there is a timeout.	
						}
