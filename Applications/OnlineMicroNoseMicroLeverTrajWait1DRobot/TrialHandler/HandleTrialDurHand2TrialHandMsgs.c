				...
				switch (*trial_status)
				{
					...
					case TRIAL_STATUS_GET_READY_TO_START:
						*trial_status = TRIAL_STATUS_IN_TRIAL;
						...
						paradigm->current_trial_data.escaping = FALSE;
						paradigm->current_trial_data.after_escaping = FALSE;
						if (paradigm->current_trial_data.catch_trial_mode_on)
						{
							if (get_rand_number() < 0.5)
								paradigm->current_trial_data.escaping = TRUE;
						}
						...
						if (paradigm->current_trial_data.escaping)
						{
							if (!write_to_trial_hand_2_exp_envi_hand_msg_buffer(msgs_trial_hand_2_exp_envi_hand, current_time, TRIAL_HAND_2_EXP_ENVI_HAND_MSG_RELEASE_PUNISHMENT, 0))
								return print_message(ERROR_MSG ,"TrialHandler", "HandleExpEnviHand2TrialHandMsgs", "handle_trial_dur_handler_to_trial_handler_msg", "write_to_trial_hand_2_exp_envi_hand_msg_buffer()");		
						}
